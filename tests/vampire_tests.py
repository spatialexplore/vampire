...
#     content = cf.generate_header_directory()
#     content += cf.generate_header_run()
#     content += cf.generate_tci_config(interval=cf.vampire.get('MODIS_PRODUCTS', 'MOD13Q1.005.interval'))
#     write_file(filename, content)
#     return None
#
...
#     data_dir = vp.get('MODIS', 'data_dir')
#     lta_dir = vp.get('MODIS', 'data_dir')
#     start_date = datetime.datetime(1981, 01, 01)
...
def test_generate_vhi():
    test_output_dir = 'S:\WFP2\PRIMA\configs'
    vp = vampire.VampireDefaults.VampireDefaults()
    start_date = datetime.datetime(2017,1,17)
    cf = vampire.MODISConfigFactory.MODISConfigFactory('', country='Sri Lanka', start_date=start_date,
                                                       end_date=start_date)
#    cf = vampire.ConfigFactory.ConfigFactory('')
    data_dir = '/srv/Vampire/data/Download'
    filename = os.path.join(test_output_dir,
                            'lka_16-day_vhi_2017-01-17.yml')
    content = cf.generate_header_directory()
    content += cf.generate_header_run()
    content += cf.generate_vci_config()
    content += cf.generate_tci_config(interval='16Days')
    content += cf.generate_vhi_config()
    write_file(filename, content)
    return None
