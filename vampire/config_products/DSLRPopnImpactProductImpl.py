...
class DSLRPopnImpactProductImpl(ImpactProductImpl.ImpactProductImpl):
    """ Initialise DSLRPopnImpactProductImpl.

    Implementation class for DSLRPopnImpactProduct.
    ...
    def __init__(self, country, valid_from_date, valid_to_date, vampire_defaults):
        super(DSLRPopnImpactProductImpl, self).__init__()
...
#             _hazard_pattern = _hazard_pattern.replace('(?P<day>\d{2})', '(?P<day>{0})'.format(self.product_date.day))
        ...
        if output_file is not None:
            ...
            if output_dir is not None:
                ...
                self.output_dir = self.vp.get('hazard_impact', 'dslr_output_dir')
                self.output_pattern = self.vp.get('hazard_impact', 'dslr_popn_output_pattern')
        ...
        self.publish_pattern = self.vp.get('hazard_impact', 'dslr_popn_pattern')
