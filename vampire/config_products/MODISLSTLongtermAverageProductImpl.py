...
class MODISLSTLongtermAverageProductImpl(RasterProductImpl.RasterProductImpl):
    ...
    def __init__(self, country, product_date, interval, vampire_defaults):
        super(MODISLSTLongtermAverageProductImpl, self).__init__()
        ...
        self.lst_dataset = BaseDataset.BaseDataset.create(dataset_type='MODIS_LST', interval=self.interval,
                                                          product_date=None,
    ...
    def generate_header(self):
        config = self.lst_dataset.generate_header()
        return config
    ...
    def generate_config(self, input_dir=None, output_dir=None, input_pattern=None,
        ...
        config = """
    ## Processing chain begin - Compute MODIS LST long-term average\n"""
        if download:
            config += self.lst_dataset.generate_config(data_dir=None, download=download)

        if self.country.lower() == 'global':
            _prefix = self.vp.get('MODIS', 'global_prefix')
            _input_pattern = self.vp.get('MODIS_LST', 'lst_average_pattern')
            ...
            _output_pattern = self.vp.get('MODIS_LST_Long_Term_Average', 'lta_regional_output_pattern')
        else:
        ...
        if input_dir is None:
            _input_dir = os.path.join(_prefix, self.vp.get('MODIS_LST', 'lst_dir_suffix'))
        else:
            _input_dir = input_dir

        if output_dir is None:
            _output_dir = os.path.join(_prefix, self.vp.get('MODIS_LST_Long_Term_Average', 'lta_dir_suffix'))
        else:
